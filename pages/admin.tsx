...
export default function Admin() {
  ...
  const [newOptionName, setNewOptionName] = useState('') // 選択肢追加用ステート
  ...
  const s: any = {
    inp: { padding: '8px', border: '1px solid #ddd', borderRadius: '6px', width: '100%', boxSizing: 'border-box', marginBottom: '8px', fontSize: '13px' },
    btn: { background: '#1f2937', color: 'white', padding: '10px 16px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' },
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{fontSize:'20px'}}>🛠 管理パネル</h1>
        {/* 復活：アプリに戻るボタン */}
        <Link href="/">
          <button style={{ ...s.btn, background: '#3b82f6', display: 'flex', alignItems: 'center', gap: '5px' }}>
            🏠 アプリに戻る
          </button>
        </Link>
      </div>
      ...
      <div style={{ display: 'flex', gap: '2px', marginBottom: '20px' }}>
        {['markets', 'categories', 'users', 'config'].map((t: any) => (
          <button key={t} onClick={() => setActiveTab(t as any)} style={s.tab(activeTab === t)}>
            {t === 'markets' ? '問い管理' : t === 'categories' ? 'カテゴリ' : t === 'users' ? 'ユーザー' : '設定'}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'config' && (
        <section style={{ background: '#f9f9f9', padding: '20px', borderRadius: '12px' }}>
          ...
          <input value={siteConfig.site_description} onChange={e => setSiteConfig({...siteConfig, site_description: e.target.value})} placeholder="サイト説明文" style={s.inp} />
          <textarea value={siteConfig.admin_message} onChange={e => setSiteConfig({...siteConfig, admin_message: e.target.value})} placeholder="通信欄メッセージ" style={{...s.inp, height:'60px'}} />
          <textarea value={siteConfig.share_text_base} onChange={e => setSiteConfig({...siteConfig, share_text_base: e.target.value})} placeholder="𝕏投稿テンプレート" style={{...s.inp, height:'60px'}} />
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td><button onClick={() => supabase.from('profiles').update({ is_hidden_from_ranking: !u.is_hidden_from_ranking }).eq('id', u.id).then(()=>fetchData())}>{u.is_hidden_from_ranking ? '非表示中' : '表示中'}</button></td>
      ...
      {activeTab === 'categories' && (
        <section>
          ...
          <div style={{marginTop:'10px', display:'flex', gap:'5px'}}>
            ...
            <button onClick={() => supabase.from('categories').insert([newCategory]).then(()=>fetchData())} style={s.btn}>追加</button>
          </div>
        </section>
      )}
              ...
              {editingId === m.id ? (
                <div>
                  ...
                  <div style={{ background: '#f9f9f9', padding: '10px', borderRadius: '8px', marginTop: '10px' }}>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', marginBottom: '5px' }}>選択肢の編集・追加</div>
                    ...
                    {/* 復活：選択肢追加フォーム */}
                    <input placeholder="+ 新しい選択肢を追加" value={newOptionName} onChange={e => setNewOptionName(e.target.value)} style={{ ...s.inp, border: '1px solid #3b82f6', marginTop: '5px' }} />
                  </div>

                  <button onClick={handleUpdateMarket} style={{...s.btn, width:'100%', background:'#10b981', marginTop:'10px'}}>変更を保存</button>
                  <button onClick={() => setEditingId(null)} style={{width:'100%', background:'none', border:'none', marginTop:'5px', color:'#999'}}>キャンセル</button>
                </div>
              ) : (
              ...
              {!m.is_resolved && (
                <div style={{marginTop:'10px', borderTop:'1px dashed #eee', paddingTop:'10px'}}>
                  {m.market_options.map((opt: any) => (
                    <button key={opt.id} onClick={() => supabase.rpc('resolve_market', { market_id_input: m.id, winning_option_id: opt.id }).then(()=>fetchData())} style={{fontSize:'10px', marginRight:'5px', padding:'4px 8px', borderRadius:'4px', border:'1px solid #ef4444', color:'#ef4444', background:'#fff', fontWeight:'bold'}}>「{opt.name}」で確定</button>
                  ))}
                </div>
              )}
            </div>
