...
export default function Home() {
  ...
  const styles: any = {
    ...
    appDesc: { fontSize: '13px', color: '#6b7280', marginTop: '8px', fontWeight: 'bold' },
    card: { background: 'white', borderRadius: '16px', marginBottom: '30px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)', overflow: 'hidden', position: 'relative', border: '1px solid #f3f4f6' },
    ...
    // 判定基準のボックスデザイン
    descBox: { fontSize: '12px', color: '#4b5563', background: '#f8fafc', padding: '15px', borderRadius: '10px', marginBottom: '20px', border: '1px solid #edf2f7', lineHeight: '1.6', whiteSpace: 'pre-wrap', position: 'relative', zIndex: 10 },
    ...
    <div style={styles.container}>
      ...
      {activeTab === 'home' && (
        <>
          <div style={{display:'flex', gap:'10px', overflowX:'auto', marginBottom:'15px', paddingBottom:'5px'}}>
            {categories.map(cat => (
              <button key={cat} onClick={()=>setActiveCategory(cat)} style={{padding:'8px 18px', borderRadius:'25px', background:activeCategory===cat?'#1f2937':'white', color:activeCategory===cat?'white':'#4b5563', border:'1px solid #ddd', fontWeight:'bold', cursor:'pointer'}}>{cat}</button>
            ))}
          </div>
            ...
            return (
              <div key={m.id} style={styles.card}>
                ...
                <div style={{padding:'20px', background: 'white'}}>
                  {/* ★【修正箇所】判定基準エリア：過去の market.description を m.description として確実に表示 */}
                  {m.description && (
                  ...
                  {active ? (
                    selectedMarketId === m.id ? (
                      ...
                      <div style={{display:'flex', gap:'12px', marginTop:'20px'}}>
                        <button onClick={()=>openMarket(m.id)} style={{flex:2, padding:'14px', background:'#2563eb', color:'white', border:'none', borderRadius:'12px', fontWeight:'bold', cursor:'pointer', fontSize:'16px'}}>⚡️ 予想に参加</button>
      ...
      {activeTab === 'ranking' && (
        <div style={{background:'white', borderRadius:'16px', padding:'25px', boxShadow:'0 4px 20px rgba(0,0,0,0.05)'}}>
           ...
           {ranking.map((u, i) => (
             <div key={u.id} style={{display:'flex', padding:'15px 0', borderBottom:'1px solid #f1f5f9', alignItems:'center'}}>
      ...
      {activeTab === 'mypage' && (
        <div>
           ...
           {myBets.map(b => (
             <div key={b.id} style={{background:'white', padding:'18px', borderRadius:'16px', marginBottom:'12px', border:'1px solid #f1f5f9', boxShadow:'0 2px 4px rgba(0,0,0,0.02)'}}>
