  ...
  return (
    <div style={s.container}>
      ...
      {showAuthModal && (
        <div style={s.modal as any}>
          <div style={{background:'white', padding:'24px', borderRadius:'20px', width:'100%', maxWidth:'380px', textAlign:'center'}}>
            ...
            <button onClick={() => setShowAuthModal(false)} style={{ background: 'none', border: 'none', color: '#999' }}>閉じる</button>
          </div>
        </div>
      )}
      ...
      <header>
        ...
        {activeTab === 'home' && (
          <><div style={{ fontSize: '11px', background: '#f0f9ff', color: '#0369a1', padding: '8px', borderRadius: '6px', marginBottom: '10px', textAlign: 'center', border: '1px solid #bae6fd' }}>{config.admin_message}</div>
      ...
      {activeTab === 'info' && (
        <div style={{ fontSize: '12px', lineHeight: '1.6', padding: '10px' }}>
          <section style={{background:'#f9f9f9', padding:'15px', borderRadius:'10px', border:'1px solid #eee', marginBottom:'20px'}}><h3>📖 ヨソるの遊び方</h3><p>未来に起こる出来事を予想し、ポイントをヨソります。的中すると、プールされたポイントから配当を獲得できます。ポイントは換金できません。</p></section>
          <section style={{background:'#f9f9f9', padding:'15px', borderRadius:'10px', border:'1px solid #eee'}}><h3>⚖️ 法的ガイドライン及び利用規約</h3><p>取得されるポイントはゲーム内通貨であり、換金機能は提供しません。刑法第185条の賭備には該当しません。ランキング上位者への賞品提供は景品表示法の範囲内で行います。デジタル特典等の付与を含む可能性があります。</p></section>
