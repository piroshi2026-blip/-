...
export default function Home() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      fetchMarkets(); setIsLoading(false)
    }
    init()
  ...
  const s: any = {
    ...
    catBtn: (active: boolean) => ({ padding: '6px 0', borderRadius: '4px', border: '1px solid #eee', background: active ? '#1f2937' : '#fff', color: active ? '#fff' : '#666', fontSize: '9px', fontWeight: 'bold' }),
  ...
  return (
    <div style={s.container}>
      {justVoted && <div style={s.modal as any}><div style={{background:'white', padding:'20px', borderRadius:'20px', textAlign:'center'}}><h3>🎯 ヨソりました！</h3><button onClick={() => { const text = config.share_text_base.replace('{title}', justVoted.title).replace('{option}', justVoted.option); window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(window.location.origin)}`, '_blank') }} style={{background:'#000', color:'#fff', padding:'12px', borderRadius:'8px', width:'100%', fontWeight:'bold', border:'none'}}>𝕏に投稿する</button><button onClick={() => setJustVoted(null)} style={{marginTop:'10px', background:'none', border:'none', color:'#999'}}>閉じる</button></div></div>}
      {showAuthModal && <div style={s.modal as any}><div style={{background:'white', padding:'24px', borderRadius:'20px', width:'100%', maxWidth:'380px'}}><h2>ログイン</h2><input type="email" placeholder="メール" value={email} onChange={e => setEmail(e.target.value)} style={{width:'100%', padding:'10px', marginBottom:'8px'}} /><input type="password" placeholder="パス" value={password} onChange={e => setPassword(e.target.value)} style={{width:'100%', padding:'10px', marginBottom:'16px'}} /><div style={{display:'flex', gap:'8px'}}><button onClick={() => handleEmailAuth('login')} style={{flex:1, padding:'12px', background:'#3b82f6', color:'#fff', border:'none', borderRadius:'8px'}}>ログイン</button><button onClick={() => handleEmailAuth('signup')} style={{flex:1, padding:'12px', background:'#1f2937', color:'#fff', border:'none', borderRadius:'8px'}}>新規登録</button></div><button onClick={() => { supabase.auth.signInAnonymously(); setShowAuthModal(false); }} style={{width:'100%', marginTop:'10px', background:'none', border:'none', color:'#999'}}>ゲスト利用</button></div></div>}
      ...
      <header>
        ...
        {activeTab === 'home' && (
          <><div style={s.adminMsg}>{config.admin_message}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '10px' }}>{['new', 'deadline'].map(t => <button key={t} onClick={() => setSortBy(t as any)} style={{ padding: '4px 12px', borderRadius: '15px', border: 'none', background: sortBy === t ? '#3b82f6' : '#eee', color: sortBy === t ? '#fff' : '#666', fontSize: '10px', fontWeight: 'bold' }}>{t === 'new' ? '✨新着' : '⏰締切'}</button>)}</div></>
        )}
      </header>
      ...
      {activeTab === 'info' && (
        <div style={{ fontSize: '12px', lineHeight: '1.6', padding: '10px' }}>
          <section style={{background:'#f9f9f9', padding:'15px', borderRadius:'10px', border:'1px solid #eee', marginBottom:'20px'}}>
            <h3 style={{borderBottom:'2px solid #3b82f6', paddingBottom:'5px'}}>ヨソるの遊び方</h3>
            <p>1. 未来に起こる出来事を予想し、ポイントをヨソ（ベット）ります。<br/>2. 予想が的中すると、プールされたポイントから配当を獲得できます。<br/>3. ポイントはゲーム内通貨であり、換金はできません。</p>
          </section>
          <section style={{ background: '#f9f9f9', padding: '15px', borderRadius: '10px', border: '1px solid #eee' }}>
            ...
            <p>取得されるポイントはゲーム内通貨であり、金銭への換金機能は提供しません。本サービスは刑法第185条の賭備には該当しません。ランキング上位者等へ賞品やデジタル特典の提供を行う場合は、景品表示法が定める「懸賞」の限度額の範囲内で行うものとします。</p>
          </section>
          <div style={{ textAlign: 'center', marginTop: '30px' }}><Link href="/admin" style={{ color: '#eee', textDecoration: 'none' }}>admin</Link></div>
        </div>
      )}
