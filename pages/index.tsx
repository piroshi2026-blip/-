...
export default function Home() {
  ...
  async function fetchMarkets() {
    // ★重要：SQLで追加した「description」を確実に取得するように指定
    let query = supabase.from('markets').select('id, title, description, category, end_date, image_url, total_pool, is_resolved, result_option_id, market_options(*)')
  ...
  const styles: any = {
    ...
    appTitle: { fontSize: '32px', fontWeight: '900', background: 'linear-gradient(to right, #2563eb, #9333ea)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', textAlign: 'center', margin: 0 },
    appDesc: { fontSize: '12px', color: '#6b7280', marginTop: '5px', fontWeight: 'bold', textAlign: 'center', marginBottom: '20px' },
    card: { background: 'white', borderRadius: '20px', marginBottom: '30px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)', overflow: 'hidden', border: '1px solid #f3f4f6' },
    imageArea: { height: '200px', position: 'relative', background: '#eee' },
    imageOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px', background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)' },
    // 判定基準ボックス：画像に被らないよう、白いエリアにしっかり余白を持って配置
    contentArea: { padding: '20px', background: 'white', position: 'relative', zIndex: 10 },
    descBox: { fontSize: '12px', color: '#4b5563', background: '#f8fafc', padding: '15px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #edf2f7', lineHeight: '1.6', whiteSpace: 'pre-wrap' },
      ...
      {activeTab === 'home' && (
        <>
          <div style={{display:'flex', gap:'10px', overflowX:'auto', marginBottom:'20px'}}>
            {categories.map(cat => (
              <button key={cat} onClick={()=>setActiveCategory(cat)} style={{padding:'8px 18px', borderRadius:'25px', background:activeCategory===cat?'#1f2937':'white', color:activeCategory===cat?'white':'#4b5563', border:'1px solid #ddd', fontWeight:'bold', whiteSpace:'nowrap'}}>{cat}</button>
            ))}
          </div>

          <div style={{display:'flex', justifyContent:'center', gap:'15px', marginBottom:'20px'}}>
            <button onClick={()=>setSortBy('popular')} style={{fontSize:'12px', border:'none', background:sortBy==='popular'?'#2563eb':'#f3f4f6', color:sortBy==='popular'?'white':'#94a3b8', padding:'6px 15px', borderRadius:'20px', fontWeight:'bold'}}>🔥 人気順</button>
            <button onClick={()=>setSortBy('deadline')} style={{fontSize:'12px', border:'none', background:sortBy==='deadline'?'#2563eb':'#f3f4f6', color:sortBy==='deadline'?'white':'#94a3b8', padding:'6px 15px', borderRadius:'20px', fontWeight:'bold'}}>⏰ 締切順</button>
          </div>
            ...
            return (
              <div key={m.id} style={styles.card}>
                <div style={styles.imageArea}>
                  {m.image_url ? <img src={m.image_url} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <div style={{textAlign:'center', paddingTop:'70px', fontSize:'50px'}}>{catInfo.icon}</div>}
                  <div style={styles.imageOverlay}>
                    ...
                    <h2 style={{color:'white', margin:0, fontSize:'22px', fontWeight:'bold', textShadow:'0 2px 10px rgba(0,0,0,0.5)'}}>{m.title}</h2>
                  </div>
                </div>

                <div style={styles.contentArea}>
                  {/* ★判定基準：SQLのdescriptionカラムをここに表示 */}
                  {m.description && (
                      ...
                      <div style={{marginTop:'20px', padding:'15px', background:'#f9fafb', borderRadius:'12px', border:'1px solid #e2e8f0'}}>
                        <div style={{display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'15px'}}>
                          {m.market_options.map((o: any) => (
                            <button key={o.id} onClick={()=>setSelectedOptionId(o.id)} style={{padding:'10px 15px', borderRadius:'25px', border:selectedOptionId===o.id?'2px solid #2563eb':'1px solid #cbd5e1', background:selectedOptionId===o.id?'#eff6ff':'white', color:'black', fontWeight:'bold'}}>{o.name}</button>
                          ))}
                        </div>
                        ...
                        <div style={{display:'flex', gap:'10px'}}>
                          <button onClick={handleVote} style={{flex:2, padding:'14px', background:'#2563eb', color:'white', border:'none', borderRadius:'12px', fontWeight:'bold'}}>投票を確定</button>
                          <button onClick={()=>setSelectedMarketId(null)} style={{flex:1, background:'#e2e8f0', color:'#475569', border:'none', borderRadius:'12px', fontWeight:'bold'}}>中止</button>
                        </div>
                      </div>
                    ) : (
                      <div style={{display:'flex', gap:'12px', marginTop:'20px'}}>
                        <button onClick={()=>{ if(!session) supabase.auth.signInAnonymously().then(()=>window.location.reload()); else setSelectedMarketId(m.id) }} style={{flex:2, padding:'14px', background:'#2563eb', color:'white', border:'none', borderRadius:'12px', fontWeight:'bold', fontSize:'16px'}}>⚡️ 予想に参加</button>
                        <button onClick={()=>shareOnX(m)} style={{flex:1, background:'black', color:'white', border:'none', borderRadius:'12px', fontWeight:'bold', fontSize:'16px'}}>𝕏 シェア</button>
                      </div>
                    )
          ...
          <div style={{textAlign:'center', paddingBottom:'40px'}}><Link href="/admin" style={{color:'#cbd5e1', textDecoration:'none', fontSize:'12px'}}>Admin Login</Link></div>
        </>
      )}

      {/* ランキング・マイページタブのロジックは維持 */}
      <nav style={styles.nav}>
        <button onClick={()=>setActiveTab('home')} style={{background:'none', border:'none', color:activeTab==='home'?'#2563eb':'#94a3b8'}}><span style={{fontSize:'24px'}}>🏠</span><br/><span style={{fontSize:'10px', fontWeight:'bold'}}>ホーム</span></button>
        <button onClick={()=>setActiveTab('ranking')} style={{background:'none', border:'none', color:activeTab==='ranking'?'#2563eb':'#94a3b8'}}><span style={{fontSize:'24px'}}>👑</span><br/><span style={{fontSize:'10px', fontWeight:'bold'}}>ランク</span></button>
        <button onClick={()=>setActiveTab('mypage')} style={{background:'none', border:'none', color:activeTab==='mypage'?'#2563eb':'#94a3b8'}}><span style={{fontSize:'24px'}}>👤</span><br/><span style={{fontSize:'10px', fontWeight:'bold'}}>マイページ</span></button>
      </nav>
    </div>
  )
}
